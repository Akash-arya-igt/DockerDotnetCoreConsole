...
namespace IGT.Webjet.GALConnection
{
    public class GALProxy
    {
        ...
        public async Task<XElement> SubmitRequestAsync(GalWebMethod _methodName, string _token, XElement _request, string _filter)
        {
            string svcCredentials = Convert.ToBase64String(ASCIIEncoding.ASCII.GetBytes(UserName + ":" + Password));

            XDocument soapEnvelopeXml = GetSoapEnvelope(_methodName, _token, _request, _filter);
        ...
        public XElement GetResponseBody(GalWebMethod _method, string _rawResponse)
        {
            string xPath = string.Empty;
            XElement responseBody = null;
            XElement tempResponse = XmlHelper.RemoveAllNamespaces(XElement.Parse(_rawResponse));
            ...
            responseBody = XmlHelper.GetChildElement(tempResponse, xPath);
            return responseBody;
        }
    }
